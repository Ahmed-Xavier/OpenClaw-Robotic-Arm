...
class AuboI5Env(gym.Env):
    ...
    def step(self, action: np.ndarray = None) -> tuple:
        # Update mocap target and AG95 gripper from action
        if action is not None:
            action = np.asarray(action, dtype=np.float64)
            if len(action) >= 7:
                self._target.set_mocap_pose(self._physics, position=action[:3], quaternion=action[3:7])
            elif len(action) >= 3:
                self._target.set_mocap_pose(self._physics, position=action[:3])
            
            if len(action) >= 8:
                ctrl_val = np.clip(action[7] * 0.943 if action[7] <= 1.0 else action[7], 0.0, 0.943)
                self._physics.bind(self._gripper.actuator).ctrl = ctrl_val
